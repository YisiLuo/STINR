    ...
    def train(self, report_loss=True, step_interval=500):
        ...
        for step in tqdm(range(self.net.training_steps)):
            ...
            if step % 1000 == 0:
                ...
                X_sum = self.X.sum(dim=0).clone()
                show_gene = torch.topk(X_sum, 1).indices
                
                font = {'family' : 'Times New Roman','size' : 20}
                
                plt.figure(figsize=(10,4))
                plt.subplot(1,2,1)
                coord = self.coord.detach().clone().cpu().numpy()
                ind_show = np.where(coord[:, 2] == coord[1, -1])
                plt.scatter(self.coord[ind_show,0].detach().cpu(),
                                      self.coord[ind_show,1].detach().cpu(), 
                                      s = 5, 
                                      c=torch.sum(self.X[ind_show,:],dim=-1).detach().cpu(),
                                      cmap='RdYlBu_r')
                plt.colorbar()  
                plt.grid(False)
                plt.ylim(plt.ylim()[::-1])
                plt.tick_params('both', left = False, labelleft=False, bottom=False, labelbottom=False)
                plt.title('observed',font)
                
                plt.subplot(1,2,2)
                plt.scatter(self.coord[ind_show,0].detach().cpu(),
                                      self.coord[ind_show,1].detach().cpu(), 
                                      s = 5, 
                                      c=torch.sum(recon[ind_show,:],dim=-1).detach().cpu(),
                                      cmap='RdYlBu_r')
                plt.colorbar()  
                plt.grid(False)
                plt.ylim(plt.ylim()[::-1])
                plt.tick_params('both', left = False, labelleft=False, bottom=False, labelbottom=False)
                plt.title('reconstruct '+str(torch.norm(self.X-recon).item()),font)
                plt.show()
                ...
                for i in range(len(result)):
                    ...
                    sc.pl.spatial(result[i], img_key="lowres", 
                                  color=list(self.adata_basis.obs.index), size=1.)
